...
프로토타입은 로그인·프로필·프로젝트 목록 화면을 갖추고 있지만 전부 localStorage 목입니다.
브리지가 그걸 실제 백엔드에 연결합니다.

**로그인 화면** — `demo@odot.app / odot1234` 하드코딩 비교를 실제 `signUp` / `logIn` 으로 바꿉니다.
프로토타입에 없는 두 가지를 폼에 끼워 넣습니다 (친구 CSS 그대로):

- **회원가입 모드 전환** — 프로토타입에는 로그인만 있습니다
- **나이 입력** — 연령별 콘텐츠 검열의 기준값이라 가입 때 꼭 필요합니다

**프로필 화면** — 이름·알림 설정을 서버에 저장합니다(`PATCH /api/me`).
계정 이메일은 로그인 신원이라 읽기 전용으로 바꿉니다.
로그아웃은 실제 세션을 무효화하고 로컬 상태까지 정리합니다.

**프로젝트 화면** — 프로토타입의 목록이 로컬 상태(`state.activeProjectPicks`) 기준이라 늘 비어 보입니다.
서버 목록이 있으면 그걸 우선해서 그리고, 항목을 누르면 그 세션으로 갈아탑니다.
